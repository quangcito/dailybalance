...
import { StructuredAnswer, Source } from '@/types/conversation'; // Import Source
...
5.  Sources: An array of Source objects ({ url: string, title: string, snippet?: string }) provided by the Knowledge Layer.

Your Task:
- Review all inputs: Reasoning Output, History, Calorie Summary, and Sources.
...
- **If the Calorie Summary values (consumed, burned, net, TDEE) are available and relevant to the query, incorporate a brief, clearly formatted summary of the user's net calorie balance. Example format: "Your current balance is X net calories (Consumed: Y kcal, Burned: Z kcal, Target: TDEE kcal)." Adjust wording if TDEE or netCalories are undefined (e.g., omit Target if TDEE is missing).**
- **Acknowledge Sources:** You are provided with a list of Sources. Use the information from these sources to inform your response, but **DO NOT** insert citation markers like [1], [2], etc., into the final response text. The sources themselves will be displayed separately in the UI.
...
- Output ONLY a JSON object matching the StructuredAnswer interface: { text: string, suggestions?: string[], dataSummary?: Record<string, any> }. Do not include any other text or explanations outside the JSON structure.
    - Ensure the 'text' field contains the main conversational response, including the clearly formatted net calorie balance summary (if applicable). **Do NOT include citation markers like [1], [2] in the text.**
...
- Reasoning Output: { "insights": "Spinach is rich in iron.", "suggestions": ["Add to smoothies."], "derivedData": { ... } }
- History: [...]
- Calorie Summary: { ... }
- Sources: [ { "url": "healthline.com/spinach", "title": "Spinach Benefits" }, { "url": "webmd.com/iron", "title": "Iron Sources" } ]
- Output: { "text": "Spinach is a great source of nutrients, particularly iron. Your current balance is 1850 net calories (Consumed: 400 kcal, Burned: 0 kcal, Target: 2250 kcal). Adding spinach to your morning smoothie is an easy way to boost your intake.", "suggestions": ["Add spinach to smoothies"], "dataSummary": { ... } }

Focus on clarity, helpfulness, and maintaining conversational flow. Ensure the output JSON strictly adheres to the StructuredAnswer format, especially including the updated calorie data in dataSummary. **Do not include citation markers in the 'text' field.**`;
...
Generate the final JSON output (StructuredAnswer) based on these inputs, incorporating the net calorie balance info and citation markers ([1], [2]...) into the text, and the calorie data into dataSummary where appropriate.
`;
